...
$pdf->writeTitle("Ad Performance Report - ".$customer, 0, 'C');
$pdf->writeSubTitle('From Date: ' . $from_date . ' To Date: ' . $to_date, 0, 'C');
$pdf->Ln();


$pdf->SetWidths(array($pdf->getAsPercentage(4), $pdf->getAsPercentage(30), $pdf->getAsPercentage(50), $pdf->getAsPercentage(15)));
$pdf->Ln();
$x = 0;

$pdf->Row(array('#', 'Billboard', 'Booking', 'Total Vehicle Count'), true);
$x += 1;
foreach ($data as $key => $value) {
    ...
    $pdf->Row(array($x, $billboard, $booking, $value->total_vehicles));
    $x += 1;
}
